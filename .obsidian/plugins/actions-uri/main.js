...
function getEnabledCommunityPlugin(pluginID) {
  return isCommunityPluginEnabled(pluginID) ? {
    isSuccess: true,
    result: window.app.plugins.getPlugin(pluginID)
  } : {
    isSuccess: false,
    errorCode: 404,
...
function showBrandedNotice(msg) {
  new import_obsidian2.Notice(`[Actions URI] ${msg}`);
}
...
async function handleCreate(incomingParams) {
  ...
  if (dailyNote instanceof import_obsidian3.TFile) {
  ...
  if (!(newNote instanceof import_obsidian3.TFile)) {
    return {
      isSuccess: false,
      errorCode: 400,
      errorMessage: STRINGS.unable_to_write_note
...
var routePath7 = {
  "/open": [
    helloRoute(),
    { path: "/search", schema: searchParams, handler: handleSearch2 }
  ]
};
...
var routePath9 = {
  "/search": [
    helloRoute(),
    { path: "/all-notes", schema: defaultParams3, handler: handleSearch3 },
    { path: "/open", schema: openParams4, handler: handleOpen3 }
  ]
};
async function handleSearch3(incomingParams) {
  const params = incomingParams;
  ...
  registerRoutes(routeTree) {
    ...
    for (const [routePath12, routeSubpaths] of Object.entries(routeTree)) {
      for (const route of routeSubpaths) {
        ...
        const fullPath = (0, import_obsidian7.normalizePath)(`${URI_NAMESPACE}/${routePath12}/${path}`).replace(/\/$/, "");
